...
class TestSyntheticDataGeneration:
    ...
    def test_complex_waveform_generation(self):
        ...
        assert not np.any(np.isnan(data))
        print(
            f"Generated 50k samples x 8 channels complex waveforms in {t['elapsed']:.4f}s"
        )
    ...
    def test_step_function_generation(self):
        ...
        assert not np.any(np.isnan(data))
        print(
            f"Generated 20k samples x 2 channels step functions in {t['elapsed']:.4f}s"
        )
...
class TestPlottingPerformance:
    ...
    def test_plotting_performance_matrix(self, n_samples, n_channels, mode):
        ...
        with temp_output_dir() as output_dir:
            ...
            print(
                f"{mode} mode: {n_samples} samples x {n_channels} channels -> "
    ...
    def test_plot_size_impact(self):
        ...
        for width, height in plot_sizes:
            with temp_output_dir() as output_dir:
                ...
                pixels = width * height
                print(
                    f"Resolution {width}x{height} ({pixels/1000:.0f}k pixels): {t['elapsed']:.3f}s"
                )
...
class TestDataComplexityImpact:
    ...
    def test_signal_complexity_comparison(self):
        ...
        test_cases = [
            ...
            (
                "noisy_sine",
                lambda: generate_sine_waves(n_samples, n_channels, noise_level=0.5),
            ),
        ]
...
class TestAlignmentPerformance:
        ...
        for mode in modes:
            with timer() as t:
                aligned = align_signal_cfr(
                    video_times=video_times, sig_values=data, ratio=1.0, mode=mode
                )
    ...
    def test_alignment_scaling(self):
        ...
        for n_samples in base_samples:
            ...
            samples_per_sec = n_samples / t["elapsed"]
            print(
                f"Aligned {n_samples} samples in {t['elapsed']:.3f}s "
...
class TestMemoryUsage:
    ...
    def test_large_dataset_handling(self):
        ...
        with temp_output_dir() as output_dir:
            ...
            print(
                f"Large dataset processing: {t['elapsed']:.2f}s ({throughput:.1f} MB/s)"
            )
